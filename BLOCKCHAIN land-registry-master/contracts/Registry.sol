...
contract Registry{
    ...
    function addAdmin(string memory _name, address _adminAddr, string memory _state, string memory _district, string memory _city) external{
    ...
    function registerLand(string memory _name,string memory _location,string memory _state, string memory _district, string memory _city, uint256 _propertyId, uint _surveyNo, address _owner, uint _marketValue, uint _sqft) external onlyAdmin{
        ...
        newLandRegistry.name = _name;
        newLandRegistry.location = _location;
...
function getNameAndLocation(string memory _state, string memory _district, string memory _city, uint _surveyNo) external view returns (string memory, string memory) {
        LandDetails storage land = landDetalsMap[_state][_district][_city][_surveyNo];
        require(land.registered, "Land not registered");
        return (land.name, land.location);
    }
